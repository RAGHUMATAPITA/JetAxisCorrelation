...
void Tree_Analyzer(TString input_file, int itxtoutFile, TString out_file, TString colliding_system, int isMC)
{
  ...
  while(getline(openInputFile, file_chain))
    {
      if(is_MC && (colliding_system == "PbPb" || colliding_system == "pp"))
	{
	  file_name_vector.push_back(file_chain);
	}
      else if (!is_MC && (colliding_system == "PbPb" || colliding_system == "pp"))
	{
	  // PbPb MAOD data || pp AOD data 
  ...
  //for(int i = 0; i < 10; i++) //event loop start
    {
	  ...
	  /*
	  if(run != 306772) continue;
	  //if(lumi <= 30 || lumi >= 600) continue;
	  //if(lumi <= 700 || lumi >= 1300) continue;
	  if(lumi <= 1600 || lumi >= 1942) continue;
	  */
      //}
		  ...
		  if(jteta[j] > jet_eta_min_cut && jteta[j] < jet_eta_max_cut)
		    {
		      ...
		      hJes_CorrpT_refpT_pTCut0_refpartonB_ctbin_W->Fill(Jes_RawpT_refpt_refpartonB_ctbin, Evtw);
		      ...
		      hJer_CorrpT_refpT_pTCut0_refpartonB_ctbin_W->Fill(Jer_RawpT_refpt_refpartonB_ctbin, Evtw);
