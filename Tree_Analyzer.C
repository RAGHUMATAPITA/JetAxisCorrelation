...
void Tree_Analyzer(TString input_file, int itxtoutFile, TString out_file, TString colliding_system, int isMC)
{
  ...
  // for E and WTA Axis resolution correction 

  TFile* fEWTAFile_DEta_pp; 
  TFile* fEWTAFile_DPhi_pp;
  TFile* fEWTAFile_DEta_PbPb[NCentbin];
  TFile* fEWTAFile_DPhi_PbPb[NCentbin];

  TF1* fEWeight_DEta_PbPb[NCentbin];
  TF1* fEWeight_DPhi_PbPb[NCentbin];
  TF1* fWTAWeight_DEta_PbPb[NCentbin];
  TF1* fWTAWeight_DPhi_PbPb[NCentbin];
  ...
  // from histogram 
  TFile* fEWTAFile;   
  THnSparseD* hEWTAFile_E_DEta;                                                                                                   
  THnSparseD* hEWTAFile_E_DPhi;
  THnSparseD* hEWTAFile_WTA_DEta;                                                                                                   
  THnSparseD* hEWTAFile_WTA_DPhi;
  ...
  */
  //TFile* fEWTAFile_DR;
  //THnSparseD* hEWTAFile_DR;
  /*
  TFile* fEWTAFile_DR;
  TH1D*hEWTAFile_DR[NCentbin][jtpT_nbins];
  */

  /*
  TFile* fEWTAFile_DR;
  TH1D*hEFile_DR[NCentbin][jtpT_nbins];
  TH1D*hWTAFile_DR[NCentbin][jtpT_nbins];
  */

  TFile* fEWTAFile_DREWTA;
  TH1D*hEWTAFile_DREWTA[NCentbin][jtpT_nbins][DR_nbins];
  ...
  if(colliding_system == "pp")
      ...
      if(is_MC && is_JER_Correction)
	{
      ...
      if(is_MC && is_EWTA_Correction)
        {
	  /*
	  EWTA_File_DEta = EWTA_File_DEta_pp;

          fEWTAFile_DEta_pp = TFile::Open(Form("ReweightFile/%s.root", EWTA_File_DEta.Data()));
          fEWeight_DEta = (TF1*)fEWTAFile_DEta_pp->Get("EAxis_Fit");
          fWTAWeight_DEta = (TF1*)fEWTAFile_DEta_pp->Get("WTAAxis_Fit");

	  EWTA_File_DPhi = EWTA_File_DPhi_pp;

          fEWTAFile_DPhi_pp = TFile::Open(Form("ReweightFile/%s.root", EWTA_File_DPhi.Data()));
          fEWeight_DPhi = (TF1*)fEWTAFile_DPhi_pp->Get("EAxis_Fit");
          fWTAWeight_DPhi = (TF1*)fEWTAFile_DPhi_pp->Get("WTAAxis_Fit");
	  */
	  /*
	  // from histogram
	  fEWTAFile = TFile::Open(Form("%s", EWTA_File_DEtaPhi_pp.Data()));
	  hEWTAFile_E_DEta = (THnSparseD*)fEWTAFile->Get("Corr_Hist/hDEta_E_gn_rc_W");
	  hEWTAFile_E_DPhi = (THnSparseD*)fEWTAFile->Get("Corr_Hist/hDPhi_E_gn_rc_W");
	  hEWTAFile_WTA_DEta = (THnSparseD*)fEWTAFile->Get("Corr_Hist/hDEta_WTA_gn_rc_W");
	  hEWTAFile_WTA_DPhi = (THnSparseD*)fEWTAFile->Get("Corr_Hist/hDPhi_WTA_gn_rc_W");
	  */

	  /*
	  fEWTAFile_DR = TFile::Open(Form("%s", EWTA_File_DEtaPhi_pp.Data()));
	  hEWTAFile_DR = (THnSparseD*)fEWTAFile_DR->Get("Corr_Hist/hDR_EWTA_Gen_Reco_Ratio_W"); 
	  */
	  //fEWTAFile_DR = TFile::Open(Form("ReweightFile/%s", EWTA_File_DEtaPhi_pp.Data()));
	}
  ...
  if(colliding_system == "PbPb")
      ...
      if(is_MC) 
	{
	  ...
	  is_JES_JER = false;
	  is_Gen_Reco_Correlation = true;
	  is_JER_Correction = true;
	  is_EWTA_Correction = true;
	}
      if(!is_MC)
	{
	  is_JEU = false;
	...
	{   
	  ...
	  EWTA_File_DEta = EWTA_File_DEta_PbPb;
	  EWTA_File_DPhi = EWTA_File_DPhi_PbPb;

	  for(int ictt = 0; ictt < NCentbin-1; ictt++)
            {
	      fEWTAFile_DEta_PbPb[ictt] = TFile::Open(Form("ReweightFile/%s%d.root", EWTA_File_DEta.Data(), ictt));
	      fEWeight_DEta_PbPb[ictt] = (TF1*)fEWTAFile_DEta_PbPb[ictt]->Get("EAxis_Fit");
	      fWTAWeight_DEta_PbPb[ictt] = (TF1*)fEWTAFile_DEta_PbPb[ictt]->Get("WTAAxis_Fit");

	      fEWTAFile_DPhi_PbPb[ictt] = TFile::Open(Form("ReweightFile/%s%d.root", EWTA_File_DPhi.Data(), ictt));
	      fEWeight_DPhi_PbPb[ictt] = (TF1*)fEWTAFile_DPhi_PbPb[ictt]->Get("EAxis_Fit");
	      fWTAWeight_DPhi_PbPb[ictt] = (TF1*)fEWTAFile_DPhi_PbPb[ictt]->Get("WTAAxis_Fit");
	    }

	  /*
	  // for histogram
	  fEWTAFile = TFile::Open(Form("%s", EWTA_File_DEtaPhi_PbPb.Data()));
	  hEWTAFile_E_DEta = (THnSparseD*)fEWTAFile->Get("Corr_Hist/hDEta_E_gn_rc_W");
	  hEWTAFile_E_DPhi = (THnSparseD*)fEWTAFile->Get("Corr_Hist/hDPhi_E_gn_rc_W");
	  hEWTAFile_WTA_DEta = (THnSparseD*)fEWTAFile->Get("Corr_Hist/hDEta_WTA_gn_rc_W");
	  hEWTAFile_WTA_DPhi = (THnSparseD*)fEWTAFile->Get("Corr_Hist/hDPhi_WTA_gn_rc_W");
	  */
	  ...
	  /*
	  fEWTAFile_DR = TFile::Open(Form("ReweightFile/%s", EWTA_File_DEtaPhi_PbPb.Data()));
	  for(int ictt = 0; ictt < NCentbin-1; ictt++)
	    {
	      for(int iptt = 0; iptt < jtpT_nbins; iptt++)
		{
		  hEWTAFile_DR[ictt][iptt] = (TH1D*)fEWTAFile_DR->Get(Form("hDR_Gen_Reco_diff_fromSparse_CentBin%d_pTBin_%d", ictt, iptt));
		}
	    }
	  */
	  /*
	  ...
	  for(int ictt = 0; ictt < NCentbin-1; ictt++)
	    {
	      for(int iptt = 0; iptt < jtpT_nbins; iptt++)
		{
		  hEFile_DR[ictt][iptt] = (TH1D*)fEWTAFile_DR->Get(Form("hDR_Gen_Reco_E_fromSparse_CentBin%d_pTBin%d", ictt, iptt));
		  hWTAFile_DR[ictt][iptt] = (TH1D*)fEWTAFile_DR->Get(Form("hDR_Gen_Reco_WTA_fromSparse_CentBin%d_pTBin%d", ictt, iptt));
		}
	    }
	  */

	  fEWTAFile_DREWTA = TFile::Open(Form("ReweightFile/%s", EWTA_File_DEtaPhi_PbPb.Data()));
	  for(int ictt = 0; ictt < NCentbin-1; ictt++)
	    {
	      for(int iptt = 0; iptt < jtpT_nbins; iptt++)
		{
		  for(int idr = 0; idr < DR_nbins; idr++)
		    {
		      hEWTAFile_DREWTA[ictt][iptt][idr] = (TH1D*)fEWTAFile_DREWTA->Get(Form("hDR_Gen_Reco_EWTA_fromSparse_CentBin%d_pTBin%d_DRBin%d", ictt, iptt, idr));
		    }
		}
	    }
	}
  ...
  if(is_MC && is_EWTA_Correction)
	...
	{
	  /*
	  std::cout<<"EWTA DEta weight file is : "<<fEWTAFile_DEta_pp->GetName()<<std::endl;
	  std::cout<<"EWTA DPhi weight file is : "<<fEWTAFile_DPhi_pp->GetName()<<std::endl;
	  fEWTAFile_DEta_pp->Close();
          delete fEWTAFile_DEta_pp;

	  fEWTAFile_DPhi_pp->Close();
          delete fEWTAFile_DPhi_pp;
	  */
	  /*
	  std::cout<<"EWTA file is: "<<fEWTAFile->GetName()<<std::endl;
	  fEWTAFile->Close();
	  delete fEWTAFile;
	  */
	  /*
	    std::cout<<"EWTA file is: "<<fEWTAFile_DR->GetName()<<std::endl;
	    fEWTAFile_DR->Close();
	    delete fEWTAFile_DR;
	  */	  
	}
      else if(colliding_system == "PbPb")
	{
	  for(int ictt = 0; ictt < NCentbin-1; ictt++)
	    {
	      std::cout<<"EWTA DEta weight file is : "<<fEWTAFile_DEta_PbPb[ictt]->GetName()<<std::endl;
	      std::cout<<"EWTA DPhi weight file is : "<<fEWTAFile_DPhi_PbPb[ictt]->GetName()<<std::endl;

	      fEWTAFile_DEta_PbPb[ictt]->Close();
	      delete fEWTAFile_DEta_PbPb[ictt];

	      fEWTAFile_DPhi_PbPb[ictt]->Close();
	      delete fEWTAFile_DPhi_PbPb[ictt];
	    }
	  /*
	  std::cout<<"EWTA file is: "<<fEWTAFile->GetName()<<std::endl;
	  fEWTAFile->Close();
	  ...
	  */
	  ...
	  //std::cout<<"EWTA file is: "<<fEWTAFile_DR->GetName()<<std::endl;
	  std::cout<<"EWTA file is: "<<fEWTAFile_DREWTA->GetName()<<std::endl;
	}
    }
  ...
  TChain *trk_tree = new TChain();
  TChain *gen_tree = new TChain();

  if(is_MC && is_Gen_Reco_Correlation)
    {
      trk_tree = new TChain("ppTrack/trackTree");
  ...
  //for(int i = 0; i < 5000; i++) //event loop start
    {
      hlt_tree->GetEntry(i);
      ...
      else // if you use multiplicity 
	{
      ...
      if(is_MC && is_EWTA_Correction && colliding_system == "PbPb")
        {
	  if(ctbin == 0)
	    {
	      fEWeight_DEta = fEWeight_DEta_PbPb[0];
	      fWTAWeight_DEta = fWTAWeight_DEta_PbPb[0];

	      fEWeight_DPhi = fEWeight_DPhi_PbPb[0];
	      fWTAWeight_DPhi = fWTAWeight_DPhi_PbPb[0];
	    }
	  else if(ctbin == 1)
	    {
	      fEWeight_DEta = fEWeight_DEta_PbPb[1];
	      fWTAWeight_DEta = fWTAWeight_DEta_PbPb[1];

	      fEWeight_DPhi = fEWeight_DPhi_PbPb[1];
	      fWTAWeight_DPhi = fWTAWeight_DPhi_PbPb[1];
	    }
	  else if(ctbin == 2)
	    {
	      fEWeight_DEta = fEWeight_DEta_PbPb[2];
              fWTAWeight_DEta = fWTAWeight_DEta_PbPb[2];

              fEWeight_DPhi = fEWeight_DPhi_PbPb[2];
              fWTAWeight_DPhi = fWTAWeight_DPhi_PbPb[2];
	    }
	  else if(ctbin == 3)
	    {
	      fEWeight_DEta = fEWeight_DEta_PbPb[3];
	      fWTAWeight_DEta = fWTAWeight_DEta_PbPb[3];

              fEWeight_DPhi = fEWeight_DPhi_PbPb[3];
              fWTAWeight_DPhi = fWTAWeight_DPhi_PbPb[3];
	    }
	  else{std::cout<<"Centrality bins are more than 3"<<std::endl; break;}
	}
      ...
      for (int j = 0; j < nref; j++) //Jet loop start
	{
	  ...
	  if(is_MC)
	      ...
	      if(jteta[j] > jet_eta_min_cut && jteta[j] < jet_eta_max_cut)
		{
	      ...
	      double highest_RecoTrackpT = -999., highest_RecoTrackEta = -999., highest_RecoTrackPhi = -999.;
	      int highest_RecoTrackIndex = -999;
	      
	      std::vector<float> highest_RecoTrackpT_vec;
		  ...
		  for(int irctrk = 0; irctrk < ntrk; irctrk++)
		    {
		      ...
		      int trk_algo = (int)trkalgo[irctrk];
		      float trk_mva = (float)trkmva[irctrk];
		      ...
		      double RecoTrack_Jet_DeltaEta = jteta[j] - trk_eta;
		      double RecoTrack_Jet_DeltaPhi = TVector2::Phi_mpi_pi(jtphi[j] - trk_phi);
		      
		      double RecoTrack_Jet_DeltaR = TMath::Sqrt(pow(RecoTrack_Jet_DeltaEta,2) + pow(RecoTrack_Jet_DeltaPhi,2));
		      
		      if(RecoTrack_Jet_DeltaR >= 0.4) continue; // condition for track to be inside the jet
		      
		      highest_RecoTrackpT_vec.push_back(trk_pt);
		      
		      //if(!trk_hp) continue;
		      //if(trk_chg == 0) continue;
		      //if(trk_pt < 0.5) continue;
		      //if(trk_nhits < 11) continue;
		      //if(trk_algo == 6 && trk_mva < 0.98) continue;
		      //if(trk_chi2/trk_ndf/trk_nlayers > 0.18) continue;
		      //if(trk_pterr/trk_pt > 0.1) continue;
		      //if(trk_dxy/trk_dxyerr > 3.) continue;
		      //if(trk_dz/trk_dzerr > 3.) continue;
		      
		      find_leading_Tracks(trk_pt, trk_eta, trk_phi, irctrk, highest_RecoTrackpT, highest_RecoTrackEta, highest_RecoTrackPhi, highest_RecoTrackIndex); // function to find highet track pt, eta, phi, and corresposnding index
		      } // reco track loop end
		  */
		  std::sort(highest_RecoTrackpT_vec.begin(), highest_RecoTrackpT_vec.end());
		}

	      highest_RecoTrackpT_vec.clear();
	      ...
	      if(is_Gen_Reco_Correlation)
		{
		  ...
		  if(nrefgen == 1)
			...
			{
			  double highest_GenTrackpT = -999., highest_GenTrackEta = -999., highest_GenTrackPhi = -999.;
			  int highest_GenTrackIndex = -999;
			  
			  std::vector<float> highest_GenTrackpT_vec;

			  int idx_gn_trk = -1;			  
			  double min_DR_hrc_gn = 999999., min_Dpt_hrc_gn = 999999.;
			  /*
			  //start gen track loop within gen jet loop
			  for(int igntrk = 0; igntrk < gen_trkpt->size(); igntrk++)
			    {
			      ...
			      double GenTrack_Jet_DeltaEta = gen_jteta[idx_gen] - gentrk_eta;
			      double GenTrack_Jet_DeltaPhi = TVector2::Phi_mpi_pi(gen_jtphi[idx_gen] - gentrk_phi);
			      
			      double GenTrack_Jet_DeltaR = TMath::Sqrt(pow(GenTrack_Jet_DeltaEta,2) + pow(GenTrack_Jet_DeltaPhi,2));
			      
			      if(GenTrack_Jet_DeltaR >= 0.4) continue; // condition for track to be inside the jet
			      
			      highest_GenTrackpT_vec.push_back(gentrk_pt);
			      
			      find_leading_Tracks(gentrk_pt, gentrk_eta, gentrk_phi, igntrk, highest_GenTrackpT, highest_GenTrackEta, highest_GenTrackPhi, highest_GenTrackIndex); // function to find highet track pt, eta, phi, and corresposnding index
			  */

			      /*
			      double Dpt_hrc_gn_trk = fabs(highest_RecoTrackpT -  gentrk_pt);
			      double Deta_hrc_gn_trk = highest_RecoTrackEta - gentrk_eta;
			      double Dphi_hrc_gn_trk = TVector2::Phi_mpi_pi(highest_RecoTrackPhi - gentrk_phi);
			      double DR_hrc_gn_trk = TMath::Sqrt(pow(Deta_hrc_gn_trk,2) + pow(Dphi_hrc_gn_trk,2));

			      if(DR_hrc_gn_trk < min_DR_hrc_gn)
				{
				  min_DR_hrc_gn = DR_hrc_gn_trk;
				  min_Dpt_hrc_gn = Dpt_hrc_gn_trk;
				  idx_gn_trk = igntrk;
				}
			      */
			  //} // gen trk loop end
			
			  std::sort(highest_GenTrackpT_vec.begin(), highest_GenTrackpT_vec.end());
			  highest_GenTrackpT_vec.clear();
			  
			  double Dpt_hrc_hgn_trk = fabs(highest_RecoTrackpT -  highest_GenTrackpT);
			  double Deta_hrc_hgn_trk = highest_RecoTrackEta - highest_GenTrackEta;
			  double Dphi_hrc_hgn_trk = TVector2::Phi_mpi_pi(highest_RecoTrackPhi - highest_GenTrackPhi);
			  double DR_hrc_hgn_trk = TMath::Sqrt(pow(Deta_hrc_hgn_trk,2) + pow(Dphi_hrc_hgn_trk,2));
		
			  int ptbin_gn = hJetpTBin->FindBin(gen_jtpt[idx_gen]) -1;
			  //if(ptbin_gn < 0) ptbin_gn = 0;
			  int ptbin_rc = hJetpTBin->FindBin(jet_pt_corr) -1;
			  //if(ptbin_rc < 0) ptbin_rc = 0;
			  ...
			  if((jet_pt_corr > jet_pt_min_cut && jet_pt_corr < jet_pt_max_cut) && (jteta[idx_ref] > jet_eta_min_cut && jteta[idx_ref] < jet_eta_max_cut))
			    {
			      ...
			      double DPhi_EWTA_rc = TVector2::Phi_mpi_pi(jtphi[idx_ref] - WTAphi[idx_ref]);
			      double DR_EWTA_rc = TMath::Sqrt(pow(DEta_EWTA_rc,2)+pow(DPhi_EWTA_rc,2));

			      double DR_EWTA_axis_rc_rc[4] = {DR_EWTA_rc, (double)ptbin_rc, (double)refpartonB, (double)ctbin};
                              hDR_EWTA_Reco_Check_W->Fill(DR_EWTA_axis_rc_rc, Evtw);

			      double DR_EWTA_axis_gnptbin_rc_rc[4] = {DR_EWTA_rc, (double)ptbin_gn, (double)refpartonB, (double)ctbin};
                              hDR_EWTA_Reco_Check_gnptbin_W->Fill(DR_EWTA_axis_gnptbin_rc_rc, Evtw);

			      double DEtaPhi_rc_EWTA_axis[6] = {DEta_EWTA_rc, DPhi_EWTA_rc, (double)ptbin_rc, (double)refpartonB, (double)ctbin, jteta[idx_ref]};
			      hDEtaPhi_EWTA_rc_W->Fill(DEtaPhi_rc_EWTA_axis, Evtw);

			      double DEta_EWTA_gn = gen_jteta[idx_gen] - gen_WTAeta[idx_gen];
			      double DPhi_EWTA_gn = TVector2::Phi_mpi_pi(gen_jtphi[idx_gen] - gen_WTAphi[idx_gen]);
			      ...
			      double DR_EWTA_axis_gnrc[5] = {DR_EWTA_gn, DR_EWTA_rc, (double)ptbin_rc, (double)refpartonB, (double)ctbin};
			      hDR_EWTA_Gen_Reco_W->Fill(DR_EWTA_axis_gnrc, Evtw);

			      if(is_EWTA_Correction)
				{
				  int gn_dr_bin = hDR_EWTA_Gen_Reco_W->GetAxis(0)->FindBin(DR_EWTA_gn)-1;
				  TRandom3 *rndm_dr_ewta = new TRandom3(0);
				  double DR_EWTA_smeared_gn = hEWTAFile_DREWTA[ctbin][ptbin_rc][gn_dr_bin]->GetRandom(rndm_dr_ewta);
				  //std::cout<<DR_EWTA_gn<<"  "<<gn_dr_bin<<"  "<<hEWTAFile_DREWTA[ctbin][ptbin_rc][gn_dr_bin]->GetName()<<"  "<<DR_EWTA_smeared_gn<<std::endl;
				  double DR_EWTA_axis_smearedgnrc[5] = {DR_EWTA_smeared_gn, DR_EWTA_rc, (double)ptbin_rc, (double)refpartonB, (double)ctbin};
				  hDR_EWTA_Gen_Reco_Check_W->Fill(DR_EWTA_axis_smearedgnrc, Evtw);
				}

			      double DR_EWTA_Ratio_gnrc = DR_EWTA_rc-DR_EWTA_gn;
			      //double DR_EWTA_Ratio_gnrc = DR_EWTA_rc/DR_EWTA_gn;
			      //double DR_EWTA_axis_Ratio_gnrc[4] = {DR_EWTA_Ratio_gnrc, gen_jtpt[idx_gen], (double)refpartonB, (double)ctbin};
			  ...
			  double ref_jet_pt_gn = gen_jtpt[idx_gen];
			  double ref_jet_eta_gn = gen_jteta[idx_gen];
			  double ref_jet_phi_gn = gen_jtphi[idx_gen];
			  double ref_jet_WTAeta_gn = gen_WTAeta[idx_gen];
			  double ref_jet_WTAphi_gn = gen_WTAphi[idx_gen];

			  double ref_jet_eta = gen_jteta[idx_gen];
			  double ref_jet_phi = gen_jtphi[idx_gen];
			  
			  double ref_jet_WTAeta = gen_WTAeta[idx_gen];
			  double ref_jet_WTAphi = gen_WTAphi[idx_gen];
			  ...
			  if(is_JER_Correction)
			    {
			      ...
			      ref_jet_pt_gn = gen_jtpt[idx_gen]*(gRandom->Gaus(1., fJERWeight->Eval(gen_jtpt[idx_gen])));
			      while(ref_jet_pt_gn < 0) ref_jet_pt_gn = gen_jtpt[idx_gen]*(gRandom->Gaus(1., fJERWeight->Eval(gen_jtpt[idx_gen])));
			    }

			  int ptbin_gn_check = hJetpTBin->FindBin(ref_jet_pt_gn) -1;
			  ...
			  if(is_EWTA_Correction)
			    {
			      ...
			      ref_jet_eta = (gen_jteta[idx_gen])+(gRandom->Gaus(0., fEWeight_DEta->Eval(gen_jtpt[idx_gen])));
			      ref_jet_phi = (gen_jtphi[idx_gen])+(gRandom->Gaus(0., fEWeight_DPhi->Eval(gen_jtpt[idx_gen])));
			      
			      ref_jet_WTAeta = (gen_WTAeta[idx_gen])+(gRandom->Gaus(0., fWTAWeight_DEta->Eval(gen_jtpt[idx_gen])));
			      ref_jet_WTAphi = (gen_WTAphi[idx_gen])+(gRandom->Gaus(0., fWTAWeight_DPhi->Eval(gen_jtpt[idx_gen])));

			      // for filling gen check dr hist
			      ref_jet_eta_gn = (gen_jteta[idx_gen])+(gRandom->Gaus(0., fEWeight_DEta->Eval(gen_jtpt[idx_gen])));
			      ref_jet_phi_gn = (gen_jtphi[idx_gen])+(gRandom->Gaus(0., fEWeight_DPhi->Eval(gen_jtpt[idx_gen])));
			      
			      ref_jet_WTAeta_gn = (gen_WTAeta[idx_gen])+(gRandom->Gaus(0., fWTAWeight_DEta->Eval(gen_jtpt[idx_gen])));
			      ref_jet_WTAphi_gn = (gen_WTAphi[idx_gen])+(gRandom->Gaus(0., fWTAWeight_DPhi->Eval(gen_jtpt[idx_gen])));

			      // commented when use DR histogram

			      /*
			      TRandom3 * rndm = new TRandom3(-1);

			      double rndm_angle_E = rndm->Uniform(0, 360);
			      double rndm_angle_WTA = rndm->Uniform(0, 360);
			      double rndm_DR_E = hEFile_DR[ctbin][ptbin_gn]->GetRandom();
			      double rndm_DR_WTA = hWTAFile_DR[ctbin][ptbin_gn]->GetRandom();

			      ref_jet_eta = (gen_jteta[idx_gen])+ (rndm_DR_E*(TMath::Cos(rndm_angle_E)));
			      ref_jet_phi = (gen_jtphi[idx_gen])+ (rndm_DR_E*(TMath::Sin(rndm_angle_E)));
			      
			      ref_jet_WTAeta = (gen_WTAeta[idx_gen])+ (rndm_DR_WTA*(TMath::Cos(rndm_angle_WTA)));
			      ref_jet_WTAphi = (gen_WTAphi[idx_gen])+ (rndm_DR_WTA*(TMath::Sin(rndm_angle_WTA)));

			      // for filling gen check dr hist
			      ref_jet_eta_gn = (gen_jteta[idx_gen])+ (rndm_DR_E*(TMath::Cos(rndm_angle_E)));
			      ref_jet_phi_gn = (gen_jtphi[idx_gen])+ (rndm_DR_E*(TMath::Sin(rndm_angle_E)));
			      
			      ref_jet_WTAeta_gn = (gen_WTAeta[idx_gen])+ (rndm_DR_WTA*(TMath::Cos(rndm_angle_WTA)));
			      ref_jet_WTAphi_gn = (gen_WTAphi[idx_gen])+ (rndm_DR_WTA*(TMath::Sin(rndm_angle_WTA)));
			      */
			    }
			
			  double for_EtaCut = jteta[idx_ref];
			  if(is_EWTA_Correction)
			    {
			      for_EtaCut = ref_jet_eta;
			    }	  
			  
			  double DEta_gn_rc_E = (jteta[idx_ref] - gen_jteta[idx_gen]);
			  double DPhi_gn_rc_E = TVector2::Phi_mpi_pi(jtphi[idx_ref] - gen_jtphi[idx_gen]);
			  double DR_gn_rc_E = TMath::Sqrt(pow(DEta_gn_rc_E,2)+pow(DPhi_gn_rc_E,2));

			  double DEta_gn_rc_WTA = (WTAeta[idx_ref] - gen_WTAeta[idx_gen]);
			  double DPhi_gn_rc_WTA = TVector2::Phi_mpi_pi(WTAphi[idx_ref] - gen_WTAphi[idx_gen]);
			  double DR_gn_rc_WTA = TMath::Sqrt(pow(DEta_gn_rc_WTA,2)+pow(DPhi_gn_rc_WTA,2));

			  double DEta_EWTA_gn_gn = gen_jteta[idx_gen] - gen_WTAeta[idx_gen];
			  double DPhi_EWTA_gn_gn = TVector2::Phi_mpi_pi(gen_jtphi[idx_gen] - gen_WTAphi[idx_gen]);
			  double DR_EWTA_gn_gn = TMath::Sqrt(pow(DEta_EWTA_gn_gn,2)+pow(DPhi_EWTA_gn_gn,2));

			  if(is_EWTA_Correction)
			    {
			      DEta_gn_rc_E = (ref_jet_eta - gen_jteta[idx_gen]);
			      DPhi_gn_rc_E = TVector2::Phi_mpi_pi(ref_jet_phi - gen_jtphi[idx_gen]);
			      DR_gn_rc_E = TMath::Sqrt(pow(DEta_gn_rc_E,2)+pow(DPhi_gn_rc_E,2));

			      DEta_gn_rc_WTA = (ref_jet_WTAeta - gen_WTAeta[idx_gen]);
			      DPhi_gn_rc_WTA = TVector2::Phi_mpi_pi(ref_jet_WTAphi - gen_WTAphi[idx_gen]);
			      DR_gn_rc_WTA = TMath::Sqrt(pow(DEta_gn_rc_WTA,2)+pow(DPhi_gn_rc_WTA,2));
			    }

			  if((ref_jet_pt_gn > jet_pt_min_cut && ref_jet_pt_gn < jet_pt_max_cut) && (ref_jet_eta_gn > jet_eta_min_cut && ref_jet_eta_gn < jet_eta_max_cut))
			    {
			      if(is_EWTA_Correction)
				{
				  DEta_EWTA_gn_gn = ref_jet_eta_gn - ref_jet_WTAeta_gn;
				  DPhi_EWTA_gn_gn = TVector2::Phi_mpi_pi(ref_jet_phi_gn - ref_jet_WTAphi_gn);
				  DR_EWTA_gn_gn = TMath::Sqrt(pow(DEta_EWTA_gn_gn,2)+pow(DPhi_EWTA_gn_gn,2));
				  ...
				  //DR_EWTA_gn_gn = DR_EWTA_gn_gn + (hEWTAFile_DR[ctbin][ptbin_gn_check]->GetRandom());
				  //while(DR_EWTA_gn_gn < 0 )DR_EWTA_gn_gn = DR_EWTA_gn_gn + (hEWTAFile_DR[ctbin][ptbin_gn_check]->GetRandom());

				  DR_EWTA_gn_gn = DR_EWTA_gn_gn * (hEWTAFile_DR[ctbin][ptbin_gn_check]->GetRandom());
				  */
				}

			      double DR_EWTA_axis_gn_gn[4] = {DR_EWTA_gn_gn, (double)ptbin_gn_check, (double)refpartonB, (double)ctbin};
			      hDR_EWTA_Gen_Check_W->Fill(DR_EWTA_axis_gn_gn, Evtw);

			      double DEtaPhi_gn_EWTA_axis[6] = {DEta_EWTA_gn_gn, DPhi_EWTA_gn_gn, (double)ptbin_gn_check, (double)refpartonB, (double)ctbin, ref_jet_eta_gn};
			      hDEtaPhi_EWTA_gn_W->Fill(DEtaPhi_gn_EWTA_axis, Evtw);
			    }

			  if((jet_pt_corr > jet_pt_min_cut && jet_pt_corr < jet_pt_max_cut) && (for_EtaCut > jet_eta_min_cut && for_EtaCut < jet_eta_max_cut))
			    {
			      ...
			      double DR_gn_rc_E_axis[5] = {DR_gn_rc_E, (double)ptbin_gn, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};
			      ...
			      double DR_gn_rc_WTA_axis[5] = {DR_gn_rc_WTA, (double)ptbin_gn, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};
			      
			      hDEta_E_gn_rc_W->Fill(DEta_gn_rc_E_axis, Evtw);
			      hDPhi_E_gn_rc_W->Fill(DPhi_gn_rc_E_axis, Evtw);
			      hDR_E_gn_rc_W->Fill(DR_gn_rc_E_axis, Evtw);
			      
			      hDEta_WTA_gn_rc_W->Fill(DEta_gn_rc_WTA_axis, Evtw);
			      hDPhi_WTA_gn_rc_W->Fill(DPhi_gn_rc_WTA_axis, Evtw);
			      hDR_WTA_gn_rc_W->Fill(DR_gn_rc_WTA_axis, Evtw);

			      //if(idx_gn_trk >= 0) 
			      {
				//if(min_Dpt_hrc_gn < 0.1)
				if(Dpt_hrc_hgn_trk < 0.1 && DR_hrc_hgn_trk < 0.1) // 
				  {
				    //std::cout<<"Matched: "<<min_DR_hrc_gn<<" "<<min_Dpt_hrc_gn<<" "<<highest_RecoTrackpT<<" "<<highest_GenTrackpT<<" "<<(*gen_trkpt)[idx_gn_trk]<<std::endl;
				    hDEta_E_gn_rc_M_W->Fill(DEta_gn_rc_E_axis, Evtw);
				    hDPhi_E_gn_rc_M_W->Fill(DPhi_gn_rc_E_axis, Evtw);
				    
				    hDEta_WTA_gn_rc_M_W->Fill(DEta_gn_rc_WTA_axis, Evtw);
				    hDPhi_WTA_gn_rc_M_W->Fill(DPhi_gn_rc_WTA_axis, Evtw);
				  }
				//else if(min_Dpt_hrc_gn >= 0.1)
				else
				  {
				    //std::cout<<"UnMatched: "<<min_DR_hrc_gn<<" "<<min_Dpt_hrc_gn<<" "<<highest_RecoTrackpT<<" "<<highest_GenTrackpT<<" "<<(*gen_trkpt)[idx_gn_trk]<<std::endl;
				    hDEta_E_gn_rc_UM_W->Fill(DEta_gn_rc_E_axis, Evtw);
				    hDPhi_E_gn_rc_UM_W->Fill(DPhi_gn_rc_E_axis, Evtw);
				    
				    hDEta_WTA_gn_rc_UM_W->Fill(DEta_gn_rc_WTA_axis, Evtw);
				    hDPhi_WTA_gn_rc_UM_W->Fill(DPhi_gn_rc_WTA_axis, Evtw);
				  }
			      }

			      double DEta_gn_rc_EWTA_axis[6] = {DEta_gn_rc_E, DEta_gn_rc_WTA, (double)ptbin_gn, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};
			      double DPhi_gn_rc_EWTA_axis[6] = {DPhi_gn_rc_E, DPhi_gn_rc_WTA, (double)ptbin_gn, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};

			      hDEta_EWTA_gn_rc_W->Fill(DEta_gn_rc_EWTA_axis, Evtw);			      
			      hDPhi_EWTA_gn_rc_W->Fill(DPhi_gn_rc_EWTA_axis, Evtw);

			      double DEtaPhi_gn_rc_E_axis[6] = {DEta_gn_rc_E, DPhi_gn_rc_E, (double)ptbin_gn, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};
			      double DEtaPhi_gn_rc_WTA_axis[6] = {DEta_gn_rc_WTA, DPhi_gn_rc_WTA, (double)ptbin_gn, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};

			      hDEtaPhi_E_gn_rc_W->Fill(DEtaPhi_gn_rc_E_axis, Evtw);			      
			      hDEtaPhi_WTA_gn_rc_W->Fill(DEtaPhi_gn_rc_WTA_axis, Evtw);
			      			      
			      double DEta_gn_rc_hgntrk_E_axis[5] = {DEta_gn_rc_E, highest_GenTrackpT, (double)refpartonB, (double)ctbin, highest_GenTrackEta};
			      double DPhi_gn_rc_hgntrk_E_axis[5] = {DPhi_gn_rc_E, highest_GenTrackpT, (double)refpartonB, (double)ctbin, highest_GenTrackEta};
			      
			      double DEta_gn_rc_hgntrk_WTA_axis[5] = {DEta_gn_rc_WTA, highest_GenTrackpT, (double)refpartonB, (double)ctbin, highest_GenTrackEta};
			      double DPhi_gn_rc_hgntrk_WTA_axis[5] = {DPhi_gn_rc_WTA, highest_GenTrackpT, (double)refpartonB, (double)ctbin, highest_GenTrackEta};

			      hDEta_E_gn_rc_hgntrk_W->Fill(DEta_gn_rc_hgntrk_E_axis, Evtw);
			      hDPhi_E_gn_rc_hgntrk_W->Fill(DPhi_gn_rc_hgntrk_E_axis, Evtw);

			      hDEta_WTA_gn_rc_hgntrk_W->Fill(DEta_gn_rc_hgntrk_WTA_axis, Evtw);
			      hDPhi_WTA_gn_rc_hgntrk_W->Fill(DPhi_gn_rc_hgntrk_WTA_axis, Evtw);

			      double DEta_gn_rc_hrctrk_E_axis[5] = {DEta_gn_rc_E, highest_RecoTrackpT, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};
			      double DPhi_gn_rc_hrctrk_E_axis[5] = {DPhi_gn_rc_E, highest_RecoTrackpT, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};
			      
			      double DEta_gn_rc_hrctrk_WTA_axis[5] = {DEta_gn_rc_WTA, highest_RecoTrackpT, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};
			      double DPhi_gn_rc_hrctrk_WTA_axis[5] = {DPhi_gn_rc_WTA, highest_RecoTrackpT, (double)refpartonB, (double)ctbin, gen_jteta[idx_gen]};
			      hDEta_E_gn_rc_hrctrk_W->Fill(DEta_gn_rc_hrctrk_E_axis, Evtw);
			      hDPhi_E_gn_rc_hrctrk_W->Fill(DPhi_gn_rc_hrctrk_E_axis, Evtw);

			      hDEta_WTA_gn_rc_hrctrk_W->Fill(DEta_gn_rc_hrctrk_WTA_axis, Evtw);
			      hDPhi_WTA_gn_rc_hrctrk_W->Fill(DPhi_gn_rc_hrctrk_WTA_axis, Evtw);
      ...
      if(nref > 1) // condition for at least 2 jets in an event to find leading and subleading jets                    
	{
	  ...
	  for (int ign = 0; ign < (int)ngen; ign++) //gen Jet loop start                                        
	      ...
	      if(is_MC && is_JER_Correction)
		{
	      ...
	      int genptbin_gn = hJetpTBin->FindBin(gen_jtpt[ign])-1;
	      //int genptbin_gn = hJetpTBin->FindBin(gen_jet_pt)-1;
	      //if(genptbin_gn < 0) genptbin_gn = 0;
	      ...
	      if(is_MC && is_EWTA_Correction)
		{
		  //if((gen_jet_pt > jet_pt_min_cut && gen_jet_pt < jet_pt_max_cut) && (gen_jet_eta > jet_eta_min_cut && gen_jet_eta < jet_eta_max_cut))
		    if((gen_jet_pt > jet_pt_min_cut && gen_jet_pt < jet_pt_max_cut))
		    {
		      /*
		      double max_DR_E = TMath::Sqrt(pow(fEWeight_DEta->Eval(gen_jtpt[ign]),2) + pow(fEWeight_DPhi->Eval(gen_jtpt[ign]),2));
		      double max_DR_WTA = TMath::Sqrt(pow(fWTAWeight_DEta->Eval(gen_jtpt[ign]),2) + pow(fWTAWeight_DPhi->Eval(gen_jtpt[ign]),2));

		      double rndm_angle_E = gRandom->Uniform(0, 2*TMath::Pi());
		      double rndm_angle_WTA = gRandom->Uniform(0, 2*TMath::Pi());

		      double rndm_DR_E = gRandom->Uniform(0., 1.5*max_DR_E);
		      double rndm_DR_WTA = gRandom->Uniform(0., 1.5*max_DR_WTA);

		      gen_jet_eta = (gen_jteta[ign])+ (rndm_DR_E*TMath::Cos(rndm_angle_E));
		      gen_jet_phi = (gen_jtphi[ign])+ (rndm_DR_E*TMath::Sin(rndm_angle_E));
		      
		      gen_jet_WTAeta = (gen_WTAeta[ign])+ (rndm_DR_WTA*TMath::Cos(rndm_angle_E));
		      gen_jet_WTAphi = (gen_WTAphi[ign])+ (rndm_DR_WTA*TMath::Sin(rndm_angle_E));
		      */
		      
		      gRandom->SetSeed(0); 
		      gRandom = new TRandom3(0);

		      gen_jet_eta = (gen_jteta[ign])+(gRandom->Gaus(0., fEWeight_DEta->Eval(gen_jtpt[ign])));
		      gen_jet_phi = (gen_jtphi[ign])+(gRandom->Gaus(0., fEWeight_DPhi->Eval(gen_jtpt[ign])));
		      
		      gen_jet_WTAeta = (gen_WTAeta[ign])+(gRandom->Gaus(0., fWTAWeight_DEta->Eval(gen_jtpt[ign])));
		      gen_jet_WTAphi = (gen_WTAphi[ign])+(gRandom->Gaus(0., fWTAWeight_DPhi->Eval(gen_jtpt[ign])));
		      
		      /*
		      double max_DEta_E = fEWeight_DEta->Eval(gen_jtpt[ign]);
		      double max_DPhi_E = fEWeight_DPhi->Eval(gen_jtpt[ign]);
		      double max_DEta_WTA = fWTAWeight_DEta->Eval(gen_jtpt[ign]);
		      double max_DPhi_WTA = fWTAWeight_DPhi->Eval(gen_jtpt[ign]);

		      double rndm_DEta_E = gRandom->Gaus(0., fEWeight_DEta->Eval(gen_jtpt[ign])); 
		      double rndm_DPhi_E = gRandom->Gaus(0., fEWeight_DPhi->Eval(gen_jtpt[ign])); 
		      double rndm_DEta_WTA = gRandom->Gaus(0., fWTAWeight_DEta->Eval(gen_jtpt[ign])); 
		      double rndm_DPhi_WTA = gRandom->Gaus(0., fWTAWeight_DPhi->Eval(gen_jtpt[ign])); 

		      while(TMath::Abs(rndm_DEta_E) > max_DEta_E) rndm_DEta_E = gRandom->Gaus(0., fEWeight_DEta->Eval(gen_jtpt[ign]));
		      while(TMath::Abs(rndm_DPhi_E) > max_DPhi_E) rndm_DPhi_E = gRandom->Gaus(0., fEWeight_DPhi->Eval(gen_jtpt[ign]));
		      while(TMath::Abs(rndm_DEta_WTA) > max_DEta_WTA) rndm_DEta_WTA = gRandom->Gaus(0., fWTAWeight_DEta->Eval(gen_jtpt[ign]));
		      while(TMath::Abs(rndm_DPhi_WTA) > max_DPhi_WTA) rndm_DPhi_WTA = gRandom->Gaus(0., fWTAWeight_DPhi->Eval(gen_jtpt[ign]));

		      gen_jet_eta = (gen_jteta[ign])+ (rndm_DEta_E);
		      gen_jet_phi = (gen_jtphi[ign])+ (rndm_DPhi_E);
		      
		      gen_jet_WTAeta = (gen_WTAeta[ign])+ (rndm_DEta_WTA);
		      gen_jet_WTAphi = (gen_WTAphi[ign])+ (rndm_DPhi_WTA);
		      */

		      /*
		      TRandom3 *rndm_gn_E = new TRandom3(0);
		      TRandom3 *rndm_gn_WTA = new TRandom3(0);
		      double rndm_angle_gn_E = rndm_gn_E->Uniform(0., 2.*TMath::Pi());
		      double rndm_angle_gn_WTA = rndm_gn_WTA->Uniform(0., 2.*TMath::Pi());
		      double rndm_DR_gn_E = hEFile_DR[ctbin][genptbin_gn]->GetRandom();
		      double rndm_DR_gn_WTA = hWTAFile_DR[ctbin][genptbin_gn]->GetRandom();


		      double max_DR_E = TMath::Sqrt(pow(fEWeight_DEta->Eval(gen_jtpt[ign]),2) + pow(fEWeight_DPhi->Eval(gen_jtpt[ign]),2));
		      double max_DR_WTA = TMath::Sqrt(pow(fWTAWeight_DEta->Eval(gen_jtpt[ign]),2) + pow(fWTAWeight_DPhi->Eval(gen_jtpt[ign]),2));

		      while(rndm_DR_gn_E >= 1.5*max_DR_E) rndm_DR_gn_E = hEFile_DR[ctbin][genptbin_gn]->GetRandom();
		      while(rndm_DR_gn_WTA >= 1.5*max_DR_WTA) rndm_DR_gn_WTA = hWTAFile_DR[ctbin][genptbin_gn]->GetRandom();

		      gen_jet_eta = (gen_jteta[ign])+(rndm_DR_gn_E*(TMath::Cos(rndm_angle_gn_E)));
		      gen_jet_phi = (gen_jtphi[ign])+(rndm_DR_gn_E*(TMath::Sin(rndm_angle_gn_E)));
		      
		      gen_jet_WTAeta = (gen_WTAeta[ign])+(rndm_DR_gn_WTA*(TMath::Cos(rndm_angle_gn_E)));
		      gen_jet_WTAphi = (gen_WTAphi[ign])+(rndm_DR_gn_WTA*(TMath::Sin(rndm_angle_gn_E)));
		      */

		      if(gen_jet_eta > jet_eta_min_cut && gen_jet_eta < jet_eta_max_cut)
			{
			  int genptbin_gen = hJetpTBin->FindBin(gen_jet_pt)-1;
			  
			  double DEta_EWTA_gen = gen_jteta[ign] - gen_WTAeta[ign];
			  double DPhi_EWTA_gen = TVector2::Phi_mpi_pi(gen_jtphi[ign] - gen_WTAphi[ign]);
			  
			  double DR_EWTA_gen = TMath::Sqrt(pow(DEta_EWTA_gen,2) + pow(DPhi_EWTA_gen,2));
			  
			  int gen_dr_bin = hDR_EWTA_Gen_Reco_W->GetAxis(0)->FindBin(DR_EWTA_gen)-1;
			  TRandom3 *rndm_dr_ewta_gen = new TRandom3(0);
			  double DR_EWTA_smeared_gen = hEWTAFile_DREWTA[ctbin][genptbin_gen][gen_dr_bin]->GetRandom(rndm_dr_ewta_gen);
			  double DR_EWTA_axis_gen[4] = {DR_EWTA_smeared_gen, (double)genptbin_gn, 1., (double)ctbin};
			  hDR_EWTA_Gen_Check_gnptbin_W->Fill(DR_EWTA_axis_gen, Evtw);
			}
		    }
		      
		  
		  /*
		    gen_jet_eta = (gen_jteta[ign])+(gRandom->Gaus(0., fEWeight_DEta->Eval(gen_jteta[ign])));
		    gen_jet_phi = (gen_jtphi[ign])+(gRandom->Gaus(0., fEWeight_DPhi->Eval(gen_jteta[ign])));
		    
		    gen_jet_WTAeta = (gen_WTAeta[ign])+(gRandom->Gaus(0., fWTAWeight_DEta->Eval(gen_jteta[ign])));
		    gen_jet_WTAphi = (gen_WTAphi[ign])+(gRandom->Gaus(0., fWTAWeight_DPhi->Eval(gen_jteta[ign])));
		  */
		  ...
		  if(colliding_system == "PbPb")
		    {
		      hEWTAFile_E_DEta->GetAxis(3)->SetRange(ctbin, ctbin);
		      hEWTAFile_E_DPhi->GetAxis(3)->SetRange(ctbin, ctbin);
		      
		      hEWTAFile_WTA_DEta->GetAxis(3)->SetRange(ctbin, ctbin);
		      hEWTAFile_WTA_DPhi->GetAxis(3)->SetRange(ctbin, ctbin);
		    }

		  hEWTAFile_E_DEta->GetAxis(1)->SetRange(hptbingen->FindBin(gen_jtpt[ign]+0.5), hptbingen->FindBin(gen_jtpt[ign]-0.5));
		  hEWTAFile_E_DPhi->GetAxis(1)->SetRange(hptbingen->FindBin(gen_jtpt[ign]+0.5), hptbingen->FindBin(gen_jtpt[ign]-0.5));

		  hEWTAFile_WTA_DEta->GetAxis(1)->SetRange(hptbingen->FindBin(gen_jtpt[ign]+0.5), hptbingen->FindBin(gen_jtpt[ign]-0.5));
		  hEWTAFile_WTA_DPhi->GetAxis(1)->SetRange(hptbingen->FindBin(gen_jtpt[ign]+0.5), hptbingen->FindBin(gen_jtpt[ign]-0.5));

		  h_E_DEta = (TH1D*)hEWTAFile_E_DEta->Projection(0, "E");
		  h_E_DEta->SetNameTitle(Form("h_E_DEta_CentBin%d_pTBin%d", ctbin, (int)hptbingen->FindBin(gen_jtpt[ign]+0.5)), "");
		  h_E_DPhi = (TH1D*)hEWTAFile_E_DPhi->Projection(0, "E");
		  h_E_DPhi->SetNameTitle(Form("h_E_DPhi_CentBin%d_pTBin%d", ctbin, (int)hptbingen->FindBin(gen_jtpt[ign]+0.5)), "");
		  h_WTA_DEta = (TH1D*)hEWTAFile_WTA_DEta->Projection(0, "E");
		  h_WTA_DEta->SetNameTitle(Form("h_WTA_DEta_CentBin%d_pTBin%d", ctbin, (int)hptbingen->FindBin(gen_jtpt[ign]+0.5)), "");
		  h_WTA_DPhi = (TH1D*)hEWTAFile_WTA_DPhi->Projection(0, "E");
		  h_WTA_DPhi->SetNameTitle(Form("h_WTA_DPhi_CentBin%d_pTBin%d", ctbin, (int)hptbingen->FindBin(gen_jtpt[ign]+0.5)), "");

		  gen_jet_eta = (gen_jteta[ign] + h_E_DEta->GetRandom()); 
		  gen_jet_phi = (gen_jtphi[ign] + h_E_DPhi->GetRandom());

		  gen_jet_WTAeta = (gen_WTAeta[ign] + h_WTA_DEta->GetRandom()); 
		  gen_jet_WTAphi = (gen_WTAphi[ign] + h_WTA_DPhi->GetRandom());

		  if(gen_jtpt[ign] >= 1000. || gen_jtpt[ign] <= 40.)
		    {
		      gen_jet_eta = gen_jteta[ign];
		      gen_jet_phi = gen_jtphi[ign];
		      gen_jet_WTAeta = gen_WTAeta[ign];
		      gen_jet_WTAphi = gen_WTAphi[ign];
		    }

		  //std::cout<<gen_jet_eta<<" "<<gen_jet_phi<<" "<<gen_jet_WTAeta<<" "<<gen_jet_WTAphi<<"    "<<gen_jteta[ign]<<" "<<gen_jtphi[ign]<<" "<<gen_WTAeta[ign]<<" "<<gen_WTAphi[ign]<<"  "<<h_E_DEta->GetRandom()<<" "<<h_E_DPhi->GetRandom()<<" "<<h_WTA_DEta->GetRandom()<<" "<<h_WTA_DPhi->GetRandom()<<"  "<<gen_jtpt[ign]<<std::endl;

		  delete gROOT->FindObject("hDEta_E_gn_rc_W_proj_0");
		  delete gROOT->FindObject("hDPhi_E_gn_rc_W_proj_0");
		  delete gROOT->FindObject("hDEta_WTA_gn_rc_W_proj_0");
		  delete gROOT->FindObject("hDPhi_WTA_gn_rc_W_proj_0");
		  */

		  /*
		  if(gen_jet_pt > jet_pt_min_cut && gen_jet_pt < jet_pt_max_cut)
		    {
		      int genptttbin = hJetpTBin->FindBin(gen_jet_pt)-1;
		      std::cout<<gen_jet_pt<<"  "<<genptttbin<<"  "<<ctbin<<"  "<<hEWTAFile_DR[ctbin][genptttbin]->GetRandom()<<std::endl;
		    }
		  */
		}
	      ...
	      FilteredJet_GenpT.SetPtEtaPhi(gen_jet_pt, gen_jet_eta, gen_jet_phi);
	      //FilteredJet_GenpT.SetPtEtaPhi(gen_jtpt[ign], gen_jet_eta, gen_jet_phi);
	      FilteredJet_GenpT_Vec.push_back(FilteredJet_GenpT);
	      
	      TVector3 FilteredWTAJet_GenpT;
	      FilteredWTAJet_GenpT.SetPtEtaPhi(gen_jet_pt, gen_jet_WTAeta, gen_jet_WTAphi);
	      //FilteredWTAJet_GenpT.SetPtEtaPhi(gen_jtpt[ign], gen_jet_WTAeta, gen_jet_WTAphi);
	      ...
	      if(nrfgn == 1)
		{
		  if(index_ref >= 0 && index_gen >= 0)
		      ...
		      else
			{
		      ...
		      int genptbin_gn_matched = hJetpTBin->FindBin(gen_jtpt[index_gen])-1;
		      if(genptbin_gn_matched < 0 ) genptbin_gn_matched = 0;
		      ...
		      if(is_MC && is_JER_Correction)
			{
		      ...
		      if(is_MC && is_EWTA_Correction)
			{
			  //if((gen_jtpt[index_gen] > jet_pt_min_cut && gen_jtpt[index_gen] < jet_pt_max_cut) && (gen_jteta[index_gen] > jet_eta_min_cut && gen_jteta[index_gen] < jet_eta_max_cut))

			  gRandom->SetSeed(0); 
			  gRandom = new TRandom3(0);
			  
			  //if((gen_jtpt[index_gen] > jet_pt_min_cut && gen_jtpt[index_gen] < jet_pt_max_cut) && (gen_jteta[index_gen] > -1.8 && gen_jteta[index_gen] < 1.8))
			  if((gen_jtpt[index_gen] > jet_pt_min_cut && gen_jtpt[index_gen] < jet_pt_max_cut) && (gen_jteta[index_gen] > jet_eta_min_cut && gen_jteta[index_gen] < jet_eta_max_cut))
			  //if((gen_jtpt[index_gen] > jet_pt_min_cut && gen_jtpt[index_gen] < jet_pt_max_cut))
			    {
			      gen_jet_Matchedeta = (gen_jteta[index_gen])+(gRandom->Gaus(0., fEWeight_DEta->Eval(gen_jtpt[index_gen])));
			      gen_jet_Matchedphi = (gen_jtphi[index_gen])+(gRandom->Gaus(0., fEWeight_DPhi->Eval(gen_jtpt[index_gen])));
			      
			      gen_jet_MatchedWTAeta = (gen_WTAeta[index_gen])+(gRandom->Gaus(0., fWTAWeight_DEta->Eval(gen_jtpt[index_gen])));
			      gen_jet_MatchedWTAphi = (gen_WTAphi[index_gen])+(gRandom->Gaus(0., fWTAWeight_DPhi->Eval(gen_jtpt[index_gen])));
			    }
			  /*
			  TRandom3 *rndm_gn_matched = new TRandom3(0);
			  double rndm_angle_gn_matched_E = rndm_gn_matched->Uniform(0, 360);
			  double rndm_angle_gn_matched_WTA = rndm_gn_matched->Uniform(0, 360);
			  double rndm_DR_gn_matched_E = hEFile_DR[ctbin][genptbin_gn_matched]->GetRandom();
			  double rndm_DR_gn_matched_WTA = hWTAFile_DR[ctbin][genptbin_gn_matched]->GetRandom();

			  gen_jet_Matchedeta = (gen_jteta[index_gen])+(rndm_DR_gn_matched_E*(TMath::Cos(rndm_angle_gn_matched_E)));
			  gen_jet_Matchedphi = (gen_jtphi[index_gen])+(rndm_DR_gn_matched_E*(TMath::Sin(rndm_angle_gn_matched_E)));

			  gen_jet_MatchedWTAeta = (gen_WTAeta[index_gen])+(rndm_DR_gn_matched_WTA*(TMath::Cos(rndm_angle_gn_matched_WTA)));
			  gen_jet_MatchedWTAphi = (gen_WTAphi[index_gen])+(rndm_DR_gn_matched_WTA*(TMath::Sin(rndm_angle_gn_matched_WTA)));

			  */

			  /*
			  gen_jet_Matchedeta = (gen_jteta[index_gen])+(gRandom->Gaus(0., fEWeight_DEta->Eval(gen_jteta[index_gen])));
			  gen_jet_Matchedphi = (gen_jtphi[index_gen])+(gRandom->Gaus(0., fEWeight_DPhi->Eval(gen_jteta[index_gen])));

			  gen_jet_MatchedWTAeta = (gen_WTAeta[index_gen])+(gRandom->Gaus(0., fWTAWeight_DEta->Eval(gen_jteta[index_gen])));
			  gen_jet_MatchedWTAphi = (gen_WTAphi[index_gen])+(gRandom->Gaus(0., fWTAWeight_DPhi->Eval(gen_jteta[index_gen])));
			  */
			  ...
			  hEWTAFile_E_DEta->GetAxis(1)->SetRange(hptbingen->FindBin(gen_jtpt[index_gen]+0.5), hptbingen->FindBin(gen_jtpt[index_gen]-0.5));
			  hEWTAFile_E_DPhi->GetAxis(1)->SetRange(hptbingen->FindBin(gen_jtpt[index_gen]+0.5), hptbingen->FindBin(gen_jtpt[index_gen]-0.5));
			  
			  hEWTAFile_WTA_DEta->GetAxis(1)->SetRange(hptbingen->FindBin(gen_jtpt[index_gen]+0.5), hptbingen->FindBin(gen_jtpt[index_gen]-0.5));
			  hEWTAFile_WTA_DPhi->GetAxis(1)->SetRange(hptbingen->FindBin(gen_jtpt[index_gen]+0.5), hptbingen->FindBin(gen_jtpt[index_gen]-0.5));
			  ...
			  gen_jet_Matchedeta = (gen_jteta[index_gen] + h_E_DEta->GetRandom()); 
			  gen_jet_Matchedphi = (gen_jtphi[index_gen] + h_E_DPhi->GetRandom());
			  
			  gen_jet_MatchedWTAeta = (gen_WTAeta[index_gen] + h_WTA_DEta->GetRandom()); 
			  gen_jet_MatchedWTAphi = (gen_WTAphi[index_gen] + h_WTA_DPhi->GetRandom());
			  
			  if(gen_jtpt[index_gen] >= 1000. || gen_jtpt[index_gen] <= 40.)
			    {
			      gen_jet_Matchedeta = gen_jteta[index_gen];
			      gen_jet_Matchedphi = gen_jtphi[index_gen];
			      gen_jet_MatchedWTAeta = gen_WTAeta[index_gen];
			      gen_jet_MatchedWTAphi = gen_WTAphi[index_gen];
			    }
			  */
			}
		      
		      TVector3 Matched_FilteredJet_GenpT;
		      Matched_FilteredJet_GenpT.SetPtEtaPhi(gen_jet_Mathcedpt, gen_jet_Matchedeta, gen_jet_Matchedphi);
		      Matched_FilteredJet_GenpT_Vec.push_back(Matched_FilteredJet_GenpT);

		      TVector3 Matched_FilteredWTAJet_GenpT;
		      Matched_FilteredWTAJet_GenpT.SetPtEtaPhi(gen_jet_Mathcedpt, gen_jet_MatchedWTAeta, gen_jet_MatchedWTAphi);
		      ...
		      if(gen_jet_Matchedeta > jet_eta_min_cut && gen_jet_Matchedeta < jet_eta_max_cut)
			{
	      ...
	      double Jet_GenpT_Eta_Phi_ctbin[4] = {gen_jet_pt, gen_jet_eta, gen_jet_phi, (double)ctbin};
	      double Jet_GenpT_WTAEta_WTAPhi_ctbin[4] = {gen_jet_pt, gen_jet_WTAeta, gen_jet_WTAphi, (double)ctbin};
      ...
      if(is_MC)
	{
	  ...
	  //from DR histogram EWTA correction
	  //DeltaR_corr_EWTA_WTACorrectionOnGen(FilteredJet_GenpT_Vec, FilteredWTAJet_GenpT_Vec, ctbin, Genrefparton_Vec, GenrefpartonB_Vec, jet_pt_min_cut, jet_pt_max_cut, jet_eta_min_cut, jet_eta_max_cut, genEvtw, hNGenJets_noW, hNGenJets_W, hDR_Gen_EWTA_noW, hDR_Gen_EWTA_W, hDEta_Gen_EWTA_noW, hDEta_Gen_EWTA_W, hDPhi_Gen_EWTA_noW, hDPhi_Gen_EWTA_W, hJetpTBin, hptgen, fptWeight, is_MC, false, is_ptWeight, is_EWTA_Correction, hEWTAFile_DR);
	  ...
	  //from DR histogram EWTA correction
	  //DeltaR_corr_EWTA_WTACorrectionOnGen(Matched_FilteredJet_GenpT_Vec, Matched_FilteredWTAJet_GenpT_Vec, ctbin, Matched_Genrefparton_Vec, Matched_GenrefpartonB_Vec, jet_pt_min_cut, jet_pt_max_cut, jet_eta_min_cut, jet_eta_max_cut, genEvtw, hNGenJets_Matched_noW, hNGenJets_Matched_W, hDR_Gen_EWTA_Matched_noW, hDR_Gen_EWTA_Matched_W, hDEta_Gen_EWTA_Matched_noW, hDEta_Gen_EWTA_Matched_W, hDPhi_Gen_EWTA_Matched_noW, hDPhi_Gen_EWTA_Matched_W, hJetpTBin, hptgen_Matched, fptWeight, is_MC, false, is_ptWeight, is_EWTA_Correction, hEWTAFile_DR);
