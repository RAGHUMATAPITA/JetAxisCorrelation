...
if [ $running_system == 1 ]; then
    ...
    #file="Data_MC_txtFiles/PbPBAOD_Data_jet60trig.txt"
    file="Data_MC_txtFiles/PbPBAOD_Data_jet80trig.txt"
    echo ${file}
else 
    #file="Data_MC_txtFiles/ppData2017_jet60or80Triggers.txt"
    file="Data_MC_txtFiles/ppData2017_jet80Trigger.txt"
    #file="Data_MC_txtFiles/ppref_2017_AODData.txt"
    echo ${file}
fi
...
if [ $istestrun == 1 ]; then
    if [ $isMC == 1 ]; then
	root -l -b -q "file_write_copy.C(${testRun_condition}, \"${file}\", \"$1\", \"espresso\", 2, \"/eos/user/r/rpradhan/Research/ATUIC/Check_RootFile/Jussi/MC/rootfiles\", \"$2\", \"$3\", ${mc_or_data})"
    else
	root -l -b -q "file_write_copy.C(${testRun_condition}, \"${file}\", \"$1\", \"espresso\", 2, \"/eos/user/r/rpradhan/Research/ATUIC/Check_RootFile/Jussi/Data/rootfiles\", \"$2\", \"$3\", ${mc_or_data})"
    fi
else
    if [ $isMC == 1 ]; then
